...
const double T0 = 70.0; // Temperatura al Tiempo 0 (0 minutos) (Condición de frontera inicial)
const double T5 = 45.0; // Temperatura al Tiempo 5 (5 minutos) (Condición de frontera final)
const double T_room = 15.0; // Temperatura de la habitación
...
{
    //double b = ((1.0 / 5.0) * log(6.0 / 11.0));  // O bien, descomentar para utilizar getBeta()
    double b = getBeta();
    int N = L / h;
    std::vector<double> T(N + 1);
    T[0] = 70.0;

    // Cálculo de la temperatura en el intervalo de tiempo L
    for (int i = 1; i <= N; i++)
    {
    ...
    // Cálculo adicional para el tiempo a las 9:10 am
    T[0] = T[N];
    N = L / h;
    ...
    // Cálculo adicional para el tiempo a las 9:20 am
    int N2 = L2 / h;
